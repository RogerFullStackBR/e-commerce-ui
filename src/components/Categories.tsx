...
const Categories = () => {
  ...
  const handleChange = (category: string | null) => {
    ...
    router.push(`${pathName}?${params.toString()}`, { scroll: false }); // Update the URL with the selected category
  };
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-2 bg-gray-100 p-2 rounded-lg mb-4 text-sm">
      {categories.map((category) => (
        <div
          className={`flex gap-2 items-center justify-center cursor-pointer px-2 py-1 rounded-md ${
